...
public class CodeGenerate implements Visitor<Object, Object> {
    ...
    public CodeGenerate(ErrorReporter errors) {
        this._errors = errors;
    }
...
//			f.visit(this, clas);
    ...
    public Object visitClassDecl(ClassDecl clas, Object o){
        ...
        for (MethodDecl m: clas.methodDeclList) {
            if (m.name.equals("main") && m.isStatic && !m.isPrivate && m.type.typeKind == TypeKind.VOID && m.parameterDeclList.size() == 1 && m.parameterDeclList.get(0).name.equals("args") && m.parameterDeclList.get(0).type.typeKind == TypeKind.ARRAY && ((ArrayType)m.parameterDeclList.get(0).type).eltType.typeKind == TypeKind.CLASS && ((ClassType)((ArrayType)m.parameterDeclList.get(0).type).eltType).className.spelling.equals("String")) {
...
//		boolean isStatic = false;
        ...
        if (stmt.ref instanceof IdRef) {
            ...
            if (ref.decl instanceof VarDecl) { // if assigning to local var
                offset = ((VarDecl) ref.decl).offset;
                _asm.add( new Mov_rmr( new R(Reg64.RBP, offset, Reg64.RBX))); // assign rax (stmt.val) to ref [rbp - offset]
                return null;
            } else if (ref.decl instanceof FieldDecl) { // if assigning to static var
                offset = ((FieldDecl) ref.decl).offset;
...
//		_asm.add( new Mov_rrm( new R(Reg64.RAX, Reg64.RBX) ) ); // store object's address in rbx
        ...
        qr.id.visit(this, o);
        FieldDecl fd = (FieldDecl) qr.id.decl;
    ...
    public ClassDecl visitClassType(ClassType ct, Object o){
        ...
        return (ClassDecl) ct.className.decl;
    }
    @Override
